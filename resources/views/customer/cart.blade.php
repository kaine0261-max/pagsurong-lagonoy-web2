    ...
    <div class="max-w-6xl mx-auto px-3 sm:px-4">
        <div class="bg-white rounded-lg shadow-md">
            <div class="px-3 sm:px-4 md:px-6 py-3 sm:py-4 border-b border-gray-200">
                <h1 class="text-lg sm:text-xl md:text-2xl font-bold text-gray-800">My Cart</h1>
            </div>

            @if(session('success'))
                <div class="mx-3 sm:mx-4 md:mx-6 mt-3 sm:mt-4 p-3 sm:p-4 bg-green-100 border border-green-400 text-green-700 rounded text-sm sm:text-base">
                    {{ session('success') }}
                </div>
            @endif

            @if(session('error'))
                <div class="mx-3 sm:mx-4 md:mx-6 mt-3 sm:mt-4 p-3 sm:p-4 bg-red-100 border border-red-400 text-red-700 rounded text-sm sm:text-base">
                    {{ session('error') }}
                </div>
            @endif
            ...
            @if($groupedCartItems->isEmpty())
                <div class="p-6 sm:p-8 md:p-12 text-center">
                    <div class="text-gray-400 text-4xl sm:text-5xl md:text-6xl mb-3 sm:mb-4">
                        <i class="fas fa-shopping-cart"></i>
                    </div>
                    <h3 class="text-base sm:text-lg font-medium text-gray-900 mb-2">Your cart is empty</h3>
                    <p class="text-sm sm:text-base text-gray-500 mb-4 sm:mb-6">Start shopping to add items to your cart.</p>
                    <a href="{{ route('customer.products') }}" class="inline-block bg-orange-500 text-white px-4 sm:px-6 py-2 sm:py-3 rounded-md hover:bg-orange-600 transition-colors duration-200 font-semibold text-sm sm:text-base">
                        Browse Products
                    </a>
                </div>
            @else
                @foreach($groupedCartItems as $businessId => $items)
                    ...
                    <div class="p-3 sm:p-4 md:p-6 border-b border-gray-200 last:border-b-0 business-block" data-business-id="{{ $businessId }}">

                        <h2 class="text-base sm:text-lg md:text-xl font-semibold text-gray-800 mb-3 sm:mb-4">From: {{ $business->name }}</h2>

                        <!-- Cart Items -->
                        <div class="space-y-3 sm:space-y-4 mb-4 sm:mb-6">
                            @foreach($items as $item)
                                <div class="flex items-center space-x-2 sm:space-x-3 md:space-x-4 p-2 sm:p-3 md:p-4 border border-gray-200 rounded-lg cart-item-row" data-cart-id="{{ $item->id }}" data-price="{{ $item->product->price }}">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="font-semibold text-gray-800 text-sm sm:text-base truncate">{{ $item->product->name }}</h3>
                                        @if($item->selected_flavor)
                                            <p class="text-xs sm:text-sm text-gray-500">Flavor: {{ $item->selected_flavor }}</p>
                                        @endif
                                        <p class="text-orange-500 font-bold text-sm sm:text-base">₱{{ number_format($item->product->price, 2) }}</p>
                                    </div>
                                    ...
                                    <div class="text-right hidden sm:block">
                                        <p class="font-bold text-gray-800 text-sm sm:text-base item-total" data-cart-id="{{ $item->id }}">₱{{ number_format($item->product->price * $item->quantity, 2) }}</p>
                                    </div>
